...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("pokerth_client.log"), logging.StreamHandler()],
)
...
class PokerTHProtobufClient:
    ...
    def _send_auth_request(self):
        ...
        try:
            ...
            init_message.buildId = (
                20200101  # Required field - use a reasonable build ID
            )

            # For authenticated login
            if self.password:
                init_message.login = (
                    pokerth_pb2.InitMessage.LoginType.authenticatedLogin
                )
            ...
            envelope.messageType = (
                pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_InitMessage
            )
    ...
    def _send_auth_challenge_response(self, session_id: int):
        ...
        try:
            ...
            envelope = pokerth_pb2.PokerTHMessage()
            envelope.type = (
                pokerth_pb2.PokerTHMessage.Type.Type_AuthClientResponseMessage
            )
            ...
            self._send_protobuf_message(envelope)
            logger.info(
                f"Sent authentication challenge response for session: {session_id}"
            )
    ...
    def _send_join_game_request(self, game_id, password=""):
        ...
        try:
            ...
            envelope = pokerth_pb2.PokerTHMessage()
            envelope.messageType = (
                pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_JoinExistingGameMessage
            )
    ...
    def _send_create_game_request(self, game_name, password=""):
        ...
        try:
            ...
            game_info.raiseIntervalMode = (
                pokerth_pb2.NetGameInfo.RaiseIntervalMode.raiseOnHandNum
            )
            ...
            envelope = pokerth_pb2.PokerTHMessage()
            envelope.messageType = (
                pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_JoinNewGameMessage
            )
    ...
    def _send_player_action(self, action_type: int, amount: Optional[int] = None):
        ...
        try:
            ...
            game_engine.type = (
                pokerth_pb2.GameEngineMessage.Type.GAME_ENGINE_PLAYER_ACTION
            )
            ...
            if amount is not None:
                if action_type == pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_BET:
                    ...
                    action_type
                    == pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_RAISE
                ):
    ...
    def _process_message(self, data: bytes):
        ...
        try:
            ...
            if (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_AuthServerChallengeMessage
            ):
                self._handle_auth_challenge(message.authServerChallengeMessage)
            elif (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_InitAckMessage
            ):
                self._handle_init_ack(message.initAckMessage)
            elif (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_AuthServerVerificationMessage
            ):
                ...
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_GameListNewMessage
            ):
                self._handle_game_list(message.gameListNewMessage)

            elif (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_JoinGameAckMessage
            ):
                self._handle_join_game_reply(message.joinGameAckMessage)

            elif (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_JoinGameFailedMessage
            ):
                self._handle_join_game_failed(message.joinGameFailedMessage)

            elif (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_GamePlayerJoinedMessage
            ):
                self._handle_game_player_joined(message.gamePlayerJoinedMessage)

            elif (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_HandStartMessage
            ):
                self._handle_hand_start(message.handStartMessage)

            elif (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_PlayersTurnMessage
            ):
                self._handle_players_turn(message.playersTurnMessage)

            elif (
                message.messageType
                == pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_ErrorMessage
            ):
    ...
    def _request_game_list(self):
        ...
        try:
            ...
            request.subscriptionAction = (
                pokerth_pb2.SubscriptionRequestMessage.SubscriptionAction.resubscribeGameList
            )

            # Create envelope message
            envelope = pokerth_pb2.PokerTHMessage()
            envelope.messageType = (
                pokerth_pb2.PokerTHMessage.PokerTHMessageType.Type_SubscriptionRequestMessage
            )
    ...
    def _handle_auth_verification(self, verification):
        ...
        try:
            if (
                ...
                == pokerth_pb2.AuthServerVerificationMessage.VerificationResult.SUCCESS
            ):
    ...
    def _handle_init_ack(self, init_ack):
        """
        Handle initialization acknowledgment message.

        Args:
            init_ack: InitAckMessage object
        """
        try:
            # Extract and store the player ID
            self.player_id = init_ack.yourPlayerId
            self.session_id = init_ack.yourSessionId

            logger.info(f"Initialization complete. My player ID: {self.player_id}")

            # If we have a rejoin game ID, handle it
            if init_ack.rejoinGameId:
                self.game_id = init_ack.rejoinGameId
                logger.info(f"Rejoining game {self.game_id}")
                self._send_rejoin_game_request(self.game_id)
            else:
            ...
            logger.error(f"Error handling init ack: {e}")
            import traceback
    ...
    def _handle_game_list(self, game_list_message):
        ...
        try:
            ...
            logger.info(
                f"Game: {game_name} (ID: {game_id}) - Players: {current_players}/{max_players}"
            )
    ...
    def _handle_join_game_reply(self, reply):
        ...
        try:
            ...
            logger.info(
                f"Successfully joined game {reply.gameId}"
    ...
    def _handle_create_game_reply(self, reply):
        ...
        try:
            if (
                reply.result
                == pokerth_pb2.CreateGameReplyMessage.CreateGameResult.CREATE_GAME_RESULT_SUCCESS
            ):
    ...
    def _handle_game_management(self, management_message):
        ...
        try:
            if (
                ...
                == pokerth_pb2.GameManagementMessage.Type.GAME_MANAGEMENT_PLAYER_JOINED
            ):
                player = management_message.player_joined
                logger.info(
                    f"Player joined: {player.player_name} (ID: {player.player_id})"
                )
                ...
                management_message.type
                == pokerth_pb2.GameManagementMessage.Type.GAME_MANAGEMENT_PLAYER_LEFT
            ):
                ...
                player_name = self.player_states.get(player_id, {}).get(
                ...
                management_message.type
                == pokerth_pb2.GameManagementMessage.Type.GAME_MANAGEMENT_GAME_STARTED
            ):
    ...
    def _handle_game_engine(self, engine_message):
        ...
        try:
            if (
                engine_message.type
                == pokerth_pb2.GameEngineMessage.Type.GAME_ENGINE_PLAYER_ID_LIST
            ):
                ...
                logger.info(
                    f"Player ID list for game {game_id}: {len(player_ids)} players"
                )
                ...
                engine_message.type
                == pokerth_pb2.GameEngineMessage.Type.GAME_ENGINE_NEW_HAND_CARDS
            ):
                ...
                logger.info(
                    f"Received {len(cards)} new hand cards: {[str(card) for card in self.hand_cards]}"
                )

            elif (
                engine_message.type
                == pokerth_pb2.GameEngineMessage.Type.GAME_ENGINE_DEAL_FLOP_CARDS
            ):
                # Handle flop cards
                ...
                logger.info(
                    f"Flop cards: {[str(card) for card in self.community_cards]}"
                )

            elif (
                engine_message.type
                == pokerth_pb2.GameEngineMessage.Type.GAME_ENGINE_DEAL_TURN_CARD
            ):
                # Handle turn card
                ...
                engine_message.type
                == pokerth_pb2.GameEngineMessage.Type.GAME_ENGINE_DEAL_RIVER_CARD
            ):
                # Handle river card
                ...
                engine_message.type
                == pokerth_pb2.GameEngineMessage.Type.GAME_ENGINE_NEXT_PLAYER_TO_ACT
            ):
                ...
                engine_message.type
                == pokerth_pb2.GameEngineMessage.Type.GAME_ENGINE_HAND_FINISHED
            ):
    ...
    def _handle_game_player(self, player_message):
        ...
        try:
            if (
                player_message.type
                == pokerth_pb2.GamePlayerMessage.Type.GAME_PLAYER_ACTION
            ):
                ...
                if (
                    action_type
                    == pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_FOLD
                ):
                    ...
                    action_type
                    == pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_CHECK
                ):
                    logger.info(f"Player {player_name} checks")

                elif (
                    action_type
                    == pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_CALL
                ):
                    logger.info(f"Player {player_name} calls")

                elif (
                    action_type == pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_BET
                ):
                    ...
                    action_type
                    == pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_ALL_IN
                ):
    ...
    def _handle_game_player_joined(self, message):
        ...
        try:
            ...
            if game_id == self.game_id:
                ...
                logger.info(
                    f"Player {player_id} joined the game"
                    + (" as admin" if is_admin else "")
                )
    ...
    def _handle_error(self, error_message):
        ...
        try:
            ...
            if error_code == pokerth_pb2.ErrorMessage.ErrorReason.initBlocked:
                logger.error(
                    "Your connection is blocked by the server. Try guest login or a different server."
                )
                if not self.is_guest:
    ...
    def _make_decision(self, to_call: int, min_raise: int, max_raise: int):
        ...
        try:
            hand_strength = calculate_hand_strength(
                self.hand_cards, self.community_cards, self.active_players
            )
            ...
            if self.ai_model:
                ...
                # Use simple logic
                action, amount = self._make_simple_decision(
            ...
            self._send_player_action(
                pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_FOLD
            )
            self.my_turn = False

    def _make_simple_decision(
        self, to_call: int, min_raise: int, max_raise: int
    ...
    def _convert_action_type(self, action: Action) -> int:
        ...
        return action_map.get(
            action, pokerth_pb2.PlayerAction.ActionType.PLAYER_ACTION_FOLD
        )
    ...
    def _handle_hand_start(self, message):
        ...
        try:
            ...
            if message.HasField("plainCards"):
                ...
                self.hand_cards = [Card(rank1, suit1), Card(rank2, suit2)]

                logger.info(
                    f"Hand started. Hole cards: {self.hand_cards[0]} {self.hand_cards[1]}"
                )
            ...
            if message.seatStates:
                ...
                for i, state in enumerate(message.seatStates):
                    if i in self.player_states:
                        active = (
                            state == pokerth_pb2.NetPlayerState.netPlayerStateNormal
                        )
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=7234, help="Server port (default: 7234)"
    )
    parser.add_argument(
        "--username", type=str, default="Guest", help="Username for authentication"
    )
    parser.add_argument(
        "--password", type=str, default="", help="Password for authentication"
    )
    parser.add_argument(
        "--guest", action="store_true", help="Use guest login instead of credentials"
    )
    parser.add_argument(
        "--model", type=str, default=None, help="Path to the AI model to use"
    )
    parser.add_argument(
        "--game", type=str, default=None, help="Name of the game to join or create"
    )
